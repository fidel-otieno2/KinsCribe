...
class Connection(db.Model):
    ...
    def to_dict(self):
        return {"id": self.id, "follower_id": self.follower_id, "following_id": self.following_id, "created_at": self.created_at.isoformat()}
...
class Post(db.Model):
    ...
    media_type = db.Column(db.String(20), nullable=True)  # image | video | text | carousel
    location = db.Column(db.String(200), nullable=True)
    hashtags = db.Column(db.String(500), nullable=True)
    privacy = db.Column(db.String(20), default="public")  # public | connections | family
    created_at = db.Column(db.DateTime, default=datetime.utcnow)
...
class PublicStory(db.Model):
    """24-hour disappearing public stories (outside family)"""
    ...
    sticker_data = db.Column(db.Text, nullable=True)  # JSON: polls, Q&A etc
    privacy = db.Column(db.String(20), default="public")  # public | connections
...
class StoryHighlight(db.Model):
    """Pinned story highlights on profile"""
...
class Conversation(db.Model):
    ...
    id = db.Column(db.Integer, primary_key=True)
    type = db.Column(db.String(20), default="private")  # private | family | group
